...
def train(student, train_loader, test_loader, unlabelled_train_loader, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        output = test(student, unlabelled_train_loader, epoch=epoch, save_name='Train ACC Unlabelled', args=args)
        all_acc, old_acc, new_acc = output['All'], output['Old'], output['New']
        args.logger.info('Testing on disjoint test set...')
        output_test = test(student, test_loader, epoch=epoch, save_name='Test ACC', args=args)
        all_acc_test, old_acc_test, new_acc_test = output_test['All'], output_test['Old'], output_test['New']
        
        wandb.log({k + '_train': v for k, v in output.items()})
        wandb.log({k + '_test': v for k, v in output_test.items()})
...
if __name__ == "__main__":
    ...
    parser.add_argument('--imbalance_factor', type=float, default=0.02)
    ...
    # TRAIN
    # ----------------------
    wandb.init(project='SimGCD', name=args.exp_name, tags=args.tags.split('-') if args.tags is not None else None, resume=True if args.resume is not None else None)
